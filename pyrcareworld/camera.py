...
class Camera(RCareWorldBaseObject):
    ...
    def getRGB(self, mode="wh"):
        ...
        reshaped_image_rgb = np.flipud(reshaped_image_rgb)
    ...
    def getPointCloudWithDepth(self):
        ...
        self.is_initialized.append("depth_wh")
        self.env._step()
        image_depth_exr = self.env.instance_channel.data[self.id]["depth_exr"]
    ...
    def arraysToPointCloud(self, file):
        r = self.getRGB("wh")
        d = self.getDepthEXR("wh")
        # rgb = o3d.geometry.Image(r)
        # depth = o3d.geometry.Image(d)
        extrinsic = self.getLocalToWorldMatrix()
        pcd = dp.image_array_to_point_cloud(r, d, 60, extrinsic)
