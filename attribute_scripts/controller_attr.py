...
def SetJointVelocityDirectly(kwargs: dict) -> OutgoingMessage:
    compulsory_params = ["id", "joint_velocities"]
    utility.CheckKwargs(kwargs, compulsory_params)

    msg = OutgoingMessage()
    joint_velocities = kwargs["joint_velocities"]
    num_joints = len(joint_velocities)

    msg.write_int32(kwargs["id"])
    msg.write_string("SetJointVelocity")
    msg.write_int32(num_joints)
    msg.write_float32_list(kwargs["joint_velocities"])
